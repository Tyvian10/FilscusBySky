using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

namespace FilscusBySky.Data.Migrations
{
    /// <inheritdoc />
    public partial class AddDomiciliaties : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                name: "Domiciliaties",
                columns: table => new
                {
                    Id = table.Column<int>(type: "int", nullable: false)
                        .Annotation("SqlServer:Identity", "1, 1"),
                    Naam = table.Column<string>(type: "nvarchar(200)", maxLength: 200, nullable: false),
                    Bedrag = table.Column<decimal>(type: "decimal(18,2)", precision: 18, scale: 2, nullable: false),
                    Categorie = table.Column<string>(type: "nvarchar(100)", maxLength: 100, nullable: false),
                    Dag = table.Column<int>(type: "int", nullable: false),
                    IsActief = table.Column<bool>(type: "bit", nullable: false),
                    RekeningId = table.Column<int>(type: "int", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_Domiciliaties", x => x.Id);
                    table.ForeignKey(
                        name: "FK_Domiciliaties_Rekeningen_RekeningId",
                        column: x => x.RekeningId,
                        principalTable: "Rekeningen",
                        principalColumn: "Id",
                        onDelete: ReferentialAction.Cascade);
                });

            migrationBuilder.CreateIndex(
                name: "IX_Domiciliaties_RekeningId",
                table: "Domiciliaties",
                column: "RekeningId");
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "Domiciliaties");
        }
    }
}
